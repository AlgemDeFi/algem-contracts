...
import "src/dApps/Velodrome/LFxVelodromeV3Vault0.sol";
import "src/dApps/Velodrome/LFxVelodromeV3Pool.sol";

contract LFxVelodromeV3Vault0Test is Required, Workbench {
    LFxVelodromeV3Vault0 v3vault;
    LFxVelodromeV3Pool v3pool;
    bool isWtoken0;
    ...
    function deposit(uint256 wrapped, address sender) public override {
        ...
        v3vault.calculateAddLiquidity(a0, a1);
        fund(sender, a1, a0);
        ...
        for (uint256 i; i < runs; i++) {
            {
                (bool success, bytes memory data) =
                    address(cfg.pool).staticcall(abi.encodeWithSignature("slot0()"));
    ...
    function priceImpact(address user) public {
        LFxVelodromeV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user);
        while (info.hf >= v3vault.LIQUIDATION_THRESHOLD()) {
            uint256 amnt = 10000 ether;
            vm.startPrank(cfg.pairwhale);
            pair.transfer(address(this), amnt);
            vm.stopPrank();

            (bool success, bytes memory data) =
                address(v3vault.v3pool()).staticcall(abi.encodeWithSignature("slot0()"));
            (uint160 price) = abi.decode(data, (uint160));
            ...
            v3vault.calculateRemoveLiquidity(v3vault.getUserLP(user));
        }
    }
    //^handy
    ...
    function setUp() public {
        _chooseConfig("script/cfg/Soneium/dApps/Velodrome/", 0);
    ...
    function testDepositWithdrawRedeem() public {
        ...
        console.log("kyo in vault after redeem", pair.balanceOf(address(v3vault)));
        console.log("user1 kyo balance", pair.balanceOf(address(user1)));
    ...
    function testDepositDefault(uint256 input) public {
        vm.assume(input > 100000 gwei);
        vm.assume(input < 1.5 ether);
        vm.warp(v3vault.START() + 1);
    ...
    function testLiquidation(uint256 input) public {
        ...
        v3vault.getVaultUserInfo(user1);
        v3vault.liquidate(user1);
        LFxVelodromeV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user1);
