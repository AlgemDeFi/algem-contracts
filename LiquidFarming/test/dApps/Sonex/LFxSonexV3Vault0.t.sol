...
import "src/dApps/Sonex/LFxSonexV3Vault0.sol";
import "src/dApps/Sonex/LFxSonexV3Pool.sol";

contract LFxSonexV3Vault0Test is Required, Workbench {
    LFxSonexV3Vault0 v3vault;
    LFxSonexV3Pool v3pool;
    bool isWtoken0;

    int24 maxtick = -100000;
    int24 mintick = -300000;
    ...
    function priceImpact(address user) public {
        LFxSonexV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user);
        while (info.hf >= v3vault.LIQUIDATION_THRESHOLD()) {
            uint256 amnt = 50000000;
            vm.startPrank(cfg.pairwhale);
    ...
    function setUp() public {
        _chooseConfig("script/cfg/Soneium/dApps/Sonex/", 1);
    ...
    function testLiquidation(uint256 input) public {
        ...
        v3vault.getVaultUserInfo(user1);
        v3vault.liquidate(user1);
        LFxSonexV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user1);
