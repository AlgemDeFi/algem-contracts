...
contract LFxKyoV3Vault0Test is Required, Workbench {
    LFxKyoV3Vault0 v3vault;
    LFxKyoV3Pool v3pool;
    bool isWtoken0;

    int24 maxtick = -167428;
    int24 mintick = -227428;
    ...
    function priceImpact(address user) public {
        LFxKyoV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user);
        while (info.hf >= v3vault.LIQUIDATION_THRESHOLD()) {
            uint256 amnt = 500000000;
            vm.startPrank(cfg.pairwhale);
    ...
    function setUp() public {
        _chooseConfig("script/cfg/Soneium/dApps/Kyo/", 0);
    ...
    function testLiquidation(uint256 input) public {
        ...
        v3vault.getVaultUserInfo(user1);
        v3vault.liquidate(user1);
        LFxKyoV3Vault0.userVaultInfo memory info = v3vault.getVaultUserInfo(user1);
    ...
    function testClaimMath(uint256 input) public {
        ...
        for (uint256 i = 1; i <= v3vault.totalRounds(); i++) {
            ...
            if (i == v3vault.totalRounds() / 2) {
                if (c1[0] > 0 && c2[0] > 0) require(c1[0] > c2[0]);
                if (c1[1] > 0 && c2[1] > 0) require(c1[1] > c2[1]);
            }
