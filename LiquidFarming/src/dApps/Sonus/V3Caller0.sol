...
abstract contract V3Caller0 {
    ...
    /// @return liquidity_ delta liquidity
    /// @return amount0_ actually added
    /// @return amount1_ actually added
    function increaseLiquidity(uint256 _amount0, uint256 _amount1) internal returns (uint128, uint256, uint256) {
        uint128 liquidity = calculateAddLiquidity(isWtoken0 ? _amount1 : _amount0, isWtoken0 ? _amount0 : _amount1);
        (uint256 amount0, uint256 amount1) = v3pool.mint(
            address(this), positionParameters.tickL, positionParameters.tickU, liquidity, abi.encode(address(this))
        );
        if (isWtoken0) return (liquidity, amount1, amount0);
        else return (liquidity, amount0, amount1);
    }
    ...
    function decreaseLiquidity(uint128 _liquidity) internal returns (uint256, uint256) {
        (uint256 amount0, uint256 amount1) = v3pool.burn(positionParameters.tickL, positionParameters.tickU, _liquidity);
    ...
    function _position() internal view returns (uint128, uint128, uint128) {
        bytes32 pk = keccak256(abi.encodePacked(address(this), positionParameters.tickL, positionParameters.tickU));
        (uint128 liquidity,,, uint128 tokensOwed0, uint128 tokensOwed1) = v3pool.positions(pk);
    ...
    function getPrice() internal view returns (uint160 price) {
        /// @dev for some reason simple call to slot0() reverts I believe at decoding moment
        (bool success, bytes memory data) = address(v3pool).staticcall(abi.encodeWithSignature("slot0()"));
        (price) = abi.decode(data, (uint160));
    }
}
