...
import "src/dApps/Sonus/V3Caller0.sol";
...
/// @custom:oz-upgrades-from LFxSonusV3Vault0
contract LFxSonusV3Vault0 is Vault, V3Caller0 {
    ...
    function deposit(uint256 _amount) external payable whenNotPaused updater {
        ...
        (uint128 liquidity, uint256 amount0, uint256 amount1) = increaseLiquidity(_amount, msg.value);
        ...
        emit Deposit(msg.sender, wa, liquidity);
    }
    ...
    function redeem() external nonReentrant updater {
        ...
        if (p.finish != FINISH) {
            ...
            collect(uint128(amountToken), uint128(amountWRAPPED));
    ...
    function previewALGM(address _user) internal view returns (uint256 a_) {
        ...
        for (; roundStart < cr; roundStart++ ) {
            ...
            algmRewardsShare = (positions[_user].wrapped * bonus / 100) * REWARDS_PRECISION / balance;
    ...
    function rebalance(int24 _newTickL, int24 _newTickU) external onlyOwner {
        ...
        uint256 amount1;
        if (tb > 0) {
            (amount0, amount1) = decreaseLiquidity(tb);
        }
