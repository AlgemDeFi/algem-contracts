    ...
    describe("Algem", function () {
        ...
        describe("Initial setup", function () {
            ...
            it("Should register dAPP", async () => {
                await run("registerDapp", [liquidStaking.address]);
                await wait(2000);

            });

            it("Should set reward destination to balance", async () => {
        ...
        describe("Admin functions", function () {
            describe("NDistributor", function () {
                ...
                it("Should add utility to disallow list", async () => {
                    expect(distr.connect(accs[1]).addUtilityToDissalowList("TestDisallow")).to.be.reverted;
                    expect(
    ...
    describe("SiriusAdapter", function () {
        ...
        describe("Testing addLiquidity function", function () {
            it("Adding liquidity with _autoStake = false", async function () {
                ...
                expect(adapterLpBalanceAfter - adapterLpBalanceBefore).to.be.equal(eth * 2)
                expect(poolBalanceAfter - poolBalanceBefore).to.be.equal(parseInt(eth))
                expect(parseInt(share)).to.be.equal(parseInt(eth))
            })
            ...
            it("Adding liquidity with _autoStake = true", async function () {
                ...
                expect(parseInt(adapterGaugeBalanceAfter - adapterGaugeBalanceBefore)).to.be.equal(parseInt(eth * 2))
            })
        })
        ...
        describe("Adding liquidity and receiving rewards for one user", function () {
            ...
            it("User1 added liquidity with flag 'true'. User1 gauge bal increased. Total staked increased. Adapter gauge bal increased", async function () {
                ...
                expect(gaugeBalanceAfter - gaugeBalanceBefore).to.be.equal(eth * 2)
            ...
            it("Check if one hour and one block passed", async function () {
            ...
            it("User1 added liq second time with same values. At this time flag 'false'. User1 lp bal inreased, gauge bal the same", async function () {
                ...
                expect(lpBalAfter - lpBalBefore).to.be.equal(eth * 2)
            })
        ...
        describe("Removing liquidity", function () {
            ...
            it("Withdraw LP tokens without removing liquidity", async function () {
                ...
                expect(adapterLPbalAfter - adapterLPbalBefore).to.be.equal(parseInt(eth))
            })
            ...
            it("Remove liquidity after withdraw LP", async function () {
                ...
                expect(nastrBalanceAfter - nastrBalanceBefore).to.be.equal(parseInt(amounts[1]))
                expect(astrBalanceAfter - astrBalanceBefore).to.be.gt(parseInt(amounts[0]) * 0.999)
            })
        })
        ...
        describe("Withdraw revenue", function () {
            ...
            it("One hour later", async function () {
                await ethers.provider.send('evm_increaseTime', [oneHour]);
                await ethers.provider.send('evm_mine');
            })
            ...
            it("Withdraw revenue by owner", async function () {
                ...
                expect(srsBalanceAfter - srsBalanceBefore).to.be.equal(parseInt(revenueTotal))
            })
        })
        ...
        describe("Adding LP and Gauge tokens by user", function () {
            it("Mint LP and Gauge to user", async function () {
                ...
                expect(gaugeBalAfter - gaugeBalBefore).to.be.equal(parseInt(eth))
            })
            ...
            it("Add LP with autodeposit", async function () {
                ...
                expect(gaugeBalAfter - gaugeBalBefore).to.be.equal(parseInt(lpAmount))
            })
            ...
            it("Add Gauge", async function () {
                ...
                expect(gaugeAfter - gaugeBefore).to.be.gt(0)
                expect(gaugeAfter - gaugeBefore).to.be.equal(parseInt(gaugeAmount))
            })
        })
...
const abiMinter = [
  {
    "inputs": [
      {
        "internalType": "contract MockSiriusFarm",
        "name": "_farm",
        "type": "address"
      },
      {
...
const abiAdapter = [
  ...
  },
  {
    ...
    "name": "SetAbilityToAddLpAndGauge",
    "type": "event"
  },
  {
    "anonymous": false,
  ...
  },
  {
    ...
    "outputs": [
      {
        "internalType": "contract ISiriusFarm",
        "name": "",
        "type": "address"
      }
    ],
  ...
  },
  {
    "inputs": [
      {
        "internalType": "contract ISiriusPool",
        "name": "_pool",
        "type": "address"
      },
      {
